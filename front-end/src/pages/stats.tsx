...
const Stats = () => {
  ...
  return (
    <Layout title="Product Statistics | Shopify Product Processor">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Product Statistics</h1>
      ...
      {data && data.success && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <p className="text-2xl font-bold text-blue-800">{data.data.totalProducts}</p>
              </div>
              <div className="bg-red-50 p-4 rounded-lg">
                <p className="text-sm text-red-600">Deleted Products</p>
                <p className="text-2xl font-bold text-red-800">{data.data.deletedProducts}</p>
              </div>
            </div>
          </div>
                  ...
                  <li key={product.id} className="py-3">
                    <Link href={`/products/${product.id}`} className="hover:text-blue-600">
                      <div className="flex justify-between">
                        ...
                        <div className="text-sm text-blue-600">{product.action}</div>
                      </div>
          ...
          {/* Product Types */}
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-lg font-semibold text-gray-800 mb-4">Product Types</h2>
...
export default Stats; text-gray-500 uppercase tracking-wider">Type</th>
                      <th className="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Count</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200">
                    {data.data.productTypes.map((type, index) => (
                      <tr key={index}>
                        <td className="px-4 py-2">
                          <Link
                            href={`/?productType=${encodeURIComponent(type.type)}`}
                            className="text-blue-600 hover:text-blue-800"
                          >
                            {type.type}
                          </Link>
          ...
          {/* Vendors */}
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-lg font-semibold text-gray-800 mb-4">Vendors</h2>
